...
def mmsys_get_training_set(train_dir):
    training_set={}
    nms=os.listdir(train_dir)
    for nm in nms:
        training_set[nm]=glob.glob(os.path.join(train_dir, nm, '*'))
    return training_set

def frame_path_sorting_func(path):
    frm_id = int(os.path.splitext(os.path.basename(path))[0])
    dir_nm = os.path.dirname(path)
    guided_frm_id = '{:05d}'.format(frm_id)
    return os.path.join(dir_nm, guided_frm_id)

def parse_test_file(f_test):
    seq_patterns_dict={}
    ...
    for line in lines:
        nm = line.split(':')[0][3:]
        vid_id = line[:2]
        seq_patterns_dict[nm]='*_{}_*_{}*'.format(vid_id, nm)
    return seq_patterns_dict

def mmsys_get_test_set(f_test='yt/stats/most_dets_per_vid_top10_people.txt',
                       video_folder='yt/imgs'):
    test_seq_pat_dict = parse_test_file(f_test)
    test_set_dict = {}
    for nm, pat in test_seq_pat_dict.iteritems():
        test_set_dict[nm] = sorted(glob.glob(os.path.join(video_folder, pat, '*.jpg')), key=frame_path_sorting_func)
