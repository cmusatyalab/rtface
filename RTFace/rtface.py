"""RTFace performs fast face recognition.

This module adopts a 3-tier design of tracking, detection, and recognition to
achieve real-time face recognition. Tracking process is fast and at
foreground. Detection and recognition processes are slow and therefore running
as background processes. Recognition process communicates with OpenFace
websocket server to do recognition. This module uses Dlib's correlation tracker
for tracking, Dlib's HOG + structural SVM for detection. Recognition is done
using OpenFace.
"""
import Queue
import json
...
class FaceTransformation(object):
    ...
    def __init__(self):
        ...
        if Config.ENCRYPT_DENATURED_REGION:
            if not os.path.isdir(Config.ENCRYPT_DENATURED_REGION_OUTPUT_PATH):
                ioutil.create_dir(Config.ENCRYPT_DENATURED_REGION_OUTPUT_PATH)
        ...
        openface_port = self._launch_openface()
        ...
        self.openface_client = websocketclient.OpenFaceClient(self.server_ip,
                                                              self.server_port)
    ...
    def on_recv_detection_update(self, tracker_updates, old_faces):
        valid_prev_faces = [
            face for face in old_faces if not face.low_confidence]
        detected_faces = tracker_updates['faces']
        LOG.debug(
            'bg-thread received detection # {} faces'.format(len(detected_faces)))
        ...
        for detected_face in detected_faces:
            overlaps = [iou_area(detected_face.roi, old_face.roi)
                        for old_face in valid_prev_faces]
            max_overlaps = 0.0
            if len(overlaps) > 0:
            ...
            if max_overlaps > 0.3:
                ...
                LOG.debug(
                    'bg-thread sending calling start_track for existing tracker')
                self.faces_lock.acquire()
                old_face.tracker.start_track(
                    tracker_frame, tuple_to_drectangle(detected_face.roi))
                self.faces_lock.release()
                LOG.debug(
                    'bg-thread find match frid {} --> frid {}'.format(old_face.frid, detected_face.frid))
                old_face.frid = detected_face.frid
                tracking_faces.append(old_face)
            else:
                # not matched
                LOG.debug(
                    'bg-thread no match new frid {}'.format(detected_face.frid))
...
#                tracker = create_tracker(tracker_frame, detected_face.roi, use_dlib=Config.DLIB_TRACKING)
                ...
                tracker = concurrent_track.AsyncTrackWorker()
                tracker.start()
                tracker.start_track(
                    tracker_frame, tuple_to_drectangle(detected_face.roi))
                LOG.debug(
                    'bg-thread after tracker starts {}'.format(detected_face.frid))
...
#        new_faces.extend([face for idx, face in enumerate(old_faces) if idx not in matched_old_faces_indices])
        ...
        if (isinstance(update, RecognitionRequestUpdate)):
            ...
            if (recognition_resp['type'] == FaceRecognitionServerProtocol.TYPE_frame_resp
                ...
                if (bxid in in_fly_recognition_info):
                    ...
                    LOG.error(
                        'received response but no frame info about the request')
            else:
                LOG.error(
                    'received response is not frame_resp or frame_resp success is false')
    ...
    def correct_tracking(self, stop_event=None):
        ...
        while (not stop_event.is_set()):
            ...
            if (self.correct_tracking_event.is_set()):
                try:
                    ...
                    if tracker_updates['frame'] != None:
                        ...
                        self.faces_lock.release()
                        self.remove_failed_trackers(
                            [face for face in old_faces if face not in tracking_faces])
                        self.framebuffer.update_bx(fid, new_tracking_faces)
                        LOG.debug(
                            'bg-thread updated self.faces # {} faces'.format(len(self.faces)))
                    else:
            ...
            try:
                ...
                self.on_recv_recognition_update(
                    update, in_fly_recognition_info)
            except Queue.Empty:
                pass
    ...
    def on_receive_openface_server_result(self, resp, queue=None, recognition_busy_event=None):
        ...
        if (resp_json['type'] == FaceRecognitionServerProtocol.TYPE_frame_resp):
            ...
            LOG.debug(
                'received openface server response: {}'.format(resp[:80]))
            queue.put(resp)
        ...
        if (not recognition_busy_event.is_set()):
            ...
            for idx, roi in enumerate(rois):
                ...
                recognition_queue.put(
                    RecognitionRequestUpdate(bxids[idx], roi_center))
                detection_process_openface_client.addFrameWithID(
                    face_string, 'detect', bxids[idx])
    ...
    def catchup(self, frame, rois, img_queue, bxids):
        ...
        while frame_available:
            try:
                ...
                rois = [drectangle_to_tuple(tracker.get_position())
                        for tracker in trackers]
                frame_cnt += 1
            except Queue.Empty:
    ...
    def detect(self,
        ...
        try:
            ...
            detection_process_openface_client = websocketclient.AsyncOpenFaceClientProcess(
            ...
            while (not stop_event.is_set()):
                ...
                if (len(rois) > 0):
                    ...
                    tracker_updates = {'frame': FrameTuple(
                        frame, fid), 'faces': faces}
                else:
                    tracker_updates = {'frame': None, 'faces': []}
                trackers_queue.put(tracker_updates)
                LOG.debug(
                    'put detection updates onto the queue # {} faces'.format(len(faces)))
    ...
    def track_faces(self, rgb_img, faces):
        ...
        if (len(faces) == 0):
            ...
            if tracker_type == meanshiftTracker or tracker_type == camshiftTracker:
                ...
                for face in faces:
                    conf = face.tracker.update(
                        rgb_img, face.tracker.get_position())
                    new_roi = face.tracker.get_position()
                    face.roi = drectangle_to_tuple(new_roi)
                    if face.update_tracker_failure(conf):
                        LOG.debug(
                            'frontal tracker conf too low {}'.format(conf))
                ...
                raise TypeError(
                    "unreconized tracker type: {}".format(tracker_type))
    ...
    def encrypt_output_path(self, s): return os.path.normpath(os.path.join(
        Config.ENCRYPT_DENATURED_REGION_OUTPUT_PATH, '{}.jpg'.format(s)))

    def get_timestamped_id(self, uid): return '{}_{}'.format(
    ...
    def persist_image(self, rgb_img, face_snippets):
        ...
        if Config.WRITE_PICTURE_DEBUG:
            for roi in rois:
                ...
                imwrite_rgb(self.pic_output_path(
                    str(self.frame_id) + '_track'), rgb_img)

        if Config.ENCRYPT_DENATURED_REGION:
            for idx, roi in enumerate(rois):
                denatured_region = get_image_region(rgb_img, roi)
                denatured_region = cv2.cvtColor(
                    denatured_region, cv2.COLOR_RGB2BGR)
                retval, jpeg_data = cv2.imencode('.jpg', denatured_region)
                ciphertext = encrypt.encode_aes(
                    self.cipher, jpeg_data.tobytes())
                uid = '{}-{}'.format(self.frame_id, idx)
                output_path = self.encrypt_output_path(
                    self.get_timestamped_id(uid))
...
#        faces=self.faces[::]
        ...
        if self.need_detection:
            ...
            if is_clear(rgb_img, threshold=Config.IMAGE_CLEAR_THRESHOLD):
                ...
                LOG.debug(
                    'image {} too blurry. not running detection'.format(self.frame_id))
...
#        LOG.debug('# faces returned: {}'.format(len(self.faces)))
    ...
    def train(self, rgb_frame, name):
        ...
        rois = detect_faces(rgb_frame, self.detector,
                            largest_only=True, upsample_num_times=1)
