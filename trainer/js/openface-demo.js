...
function createSocket(address, name) {
    ...
    socket.onmessage = function(e) {
        ...
        if (j.type == "NULL") {
            ...
            tok++;
        } else if (j.type == "TRAINED") {
            console.log("Training number: " + j['NUM_SAMPLES']);
            $('#tr_cnt').text(j['NUM_SAMPLES']);
        } else if (j.type == "ANNOTATED") {
            $("#detectedFaces").html(
                "<img src='" + j['content'] + "' width='150px'></img>"
            )
        } else {
...
function clearChkCallback() {
    console.log("clear chk called");
    if (socket != null) {
        var msg = {
            'type': 'CLEAR_TRAINING',
            'identity': defaultPerson
