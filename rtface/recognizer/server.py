#!/usr/bin/env python2
#
# Copyright 2016-2017 Carnegie Mellon University
#
...
from rtface.protocol import FaceRecognitionServerProtocol
from rtface.demo_config import Config # TODO: pass this as an argument
...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    ...
    def onMessage(self, payload, isBinary):
        ...
        if msg['type'] == FaceRecognitionServerProtocol.TYPE_set_state:
            ...
            name = msg['val'].encode('ascii', 'ignore')
            print
            'remove {} from {}'.format(name, people)
            ...
            try:
                ...
                people.remove(name)
                print
                'succesfully removed {}'.format(name)
                self.trainSVM()
                msg = {
    ...
    def remove_people_with_insufficient_images(self):
        ...
        for (idx, person) in enumerate(people):
            ...
            print
            '{} has {} images'.format(person, len(person_images_hashes))
            if len(person_images_hashes) < 6:
                print
                'removing {}'.format(person)
                ...
                if len(people) == 0:
                    ...
                    if mean_features != None:
                        ...
                        if dist < SINGLE_PERSON_RECOG_THRESHOLD:
                            identity = 0
                        print
                        'dist {} identity {}'.format(dist, identity)
                elif svm:
                    svm_lock.acquire()
                    ...
                    identity = maxI
                    print
                    'svm predict {} with {}'.format(identity, confidence)
